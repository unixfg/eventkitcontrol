...
Pushing a version tag such as `v1.1.0` starts
...
4. From the exact merged commit on `main`, create and push the matching tag,
   ...
   ```bash
   git tag -s v1.1.0 -m "eventkitcontrol v1.1.0"
...
```bash
shasum -a 256 -c eventkitcontrol-v1.1.0-macos-arm64.pkg.sha256
pkgutil --check-signature eventkitcontrol-v1.1.0-macos-arm64.pkg
spctl --assess --type install --verbose=4 eventkitcontrol-v1.1.0-macos-arm64.pkg
gh attestation verify eventkitcontrol-v1.1.0-macos-arm64.pkg \
  --repo unixfg/eventkitcontrol
gh attestation verify eventkitcontrol-v1.1.0-macos-arm64.pkg.sha256 \
  --repo unixfg/eventkitcontrol
```
